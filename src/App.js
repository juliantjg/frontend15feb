...
const MultipleSignIn = async () => {
  await axios.post("http://ec2-52-64-254-219.ap-southeast-2.compute.amazonaws.com:8000/api/checkToken").catch((error) => {
    localStorage.clear();
...
const AdminVerified = async () => {
  const res = await axios.get("http://ec2-52-64-254-219.ap-southeast-2.compute.amazonaws.com:8000/api/getLogoutStatus");
...
if (token) {
  ...
  AdminVerified().then((result) => {
    console.log(result);
    if (result !== "") {
      axios.get('http://ec2-52-64-254-219.ap-southeast-2.compute.amazonaws.com:8000/api/logout')
      .then (()=>{
        localStorage.clear();
        alert(result);
        window.location.reload();
